...
async function renderHistory() {
  ...
  container.innerHTML = list.map(h => {
    ...
    const src  = h.source ? (ui[`source_${h.source}`] || h.source) : '';
    const ago  = timeAgo(h.visitedAt);
    return `
    <div class="history-item" data-url="${escHtml(h.url)}" data-lat="${h.lat}" data-lng="${h.lng}">
      <span class="history-flag">🏠</span>
      <div class="history-info">
        <div class="history-name" title="${name}">${name}</div>
        <div class="history-meta">${src ? escHtml(src) + ' · ' : ''}${ago}</div>
      </div>
      ${h.url ? `<a class="history-link-btn" href="${escHtml(h.url)}" target="_blank" rel="noopener" title="591 頁面">🔗</a>` : ''}
...
function setupHistoryToggle() {
  ...
  on('btn-clear-history', async () => {
    ...
    document.getElementById('history-footer')?.classList.add('hidden');
    const arrow = document.getElementById('history-arrow');
